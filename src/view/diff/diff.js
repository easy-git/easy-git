...
class Diff {
    ...
    async SetView() {
        ...
        let init = {};
        init = await this.getDiffDefaultOptions(selectedFile, objStatus);
        // 设置html默认内容
        if (init == 'error') {
        ...
        let result = await gitRaw(this.projectPath, diff_options, 'Git文件对比', 'result');
     ...
     * @description 设置文件对比视图
     */
    async SetViewForRevision(diffParam=[], isPrevious=undefined) {
        let {selectedFile, projectPath, objStatus} = this.ProjectData;

        let init = {};
        init = await this.getDiffRevisionOptions(selectedFile, diffParam);
    ...
    /**
     * @description 获取文件历史提交commitID列表
     */
    async getFileHistoryCommitHashList() {
        let hashList = await gitFileHistoryLogHash(this.projectPath, [this.gitDiffFilePath]);
        let historyHashNum = hashList.length;
        if (historyHashNum) {
            this.webviewPanel.webView.postMessage({
                command: "LogHistoryHash",
                list: hashList,
                historyHashNum: historyHashNum
            });
        };
    };

    /**
     * @description 选择指定的commit进行对比
     */
    async selectOneRevisionToDiff() {
        let hashList = await gitFileHistoryLogHash(this.projectPath, [this.gitDiffFilePath]);
        let data = [];
        if (hashList) {
            for (let s of hashList) {
                let shortHash = s.hash.slice(0,12);
                let date = dayjs(s.date).format('YY/MM/DD HH:mm');
                data.push({
                    "label": date + ' - ' + s.msg + ' - ' + s.hash,
                    "hash": s.hash
                });
            };
        };
        ...
        let commitID = await hx.window.showQuickPick(pickerData, {
            'placeHolder': '请选择要对比的commitID...'
        }).then( (res)=> {
        ...
        if (!commitID) return;
        this.SetViewForRevision([commitID])
    };
     ...
     */
    async getDiffDefaultOptions(selectedFile, objStatus=undefined) {
        // 是否全文显示对比
        await this.getFileDiffConfig();

        let resut = await gitFileStatus(this.projectPath, selectedFile, ['-s', selectedFile]);
        // 目前仅支持对比本地有更改的文件
     ...
     * @param {Object} selectedFile
     */
    async getDiffRevisionOptions(selectedFile, hash=[]) {
        // 是否全文显示对比
        await this.getFileDiffConfig();

        let lineOption = `-U${this.isFullTextDiffFile}`;
        let options = [...['diff', lineOption], ...hash, ...[selectedFile] ];
    ...
    async handleConflict(selectedFile, options) {
        let result = await gitRaw(this.projectPath, options, '处理冲突');
        if (result == 'success') {
            this.SetView();
    ...
    async openFile(filepath) {
        ...
        let checkResult = await gitRaw(this.projectPath, ['diff', '--check', filepath], undefined, 'result');
