...
## 2.0.0
* 优化 Git克隆视图UI, 废弃webview渲染方式，加载速度更快
* 优化 Git远程仓库创建视图UI, 废弃webview渲染方式，加载速度更快

## 1.9.0
* 优化 统计功能

## 1.8.0
* 修复 Git源码视图 某些情况下，获取文件状态失败的Bug 感谢(116***@qq.com)

## 1.7.7
* 优化 Git日志查询 增加搜索建议
* 优化 源代码管理器视图 查看文件对比 增加点击区域
* 优化 分支视图 合并指定分支 弹窗确认
* 修复 源代码管理器视图 主题监听事件 增加延迟 解决某些情况下 渲染失败的Bug
...
* 优化 快速打开Git项目 支持是否将其加入到项目管理器列表
* 优化 顶部菜单【源代码管理器】 当没有焦点时，废弃init视图，改用 EasyGit.quickOpenGitProject

## 1.7.2
* 补充 源码视图 部分文件图标
* 优化 分支视图 数据加载方式，提高性能
* 调整 顶部菜单 增加快速打开Git项目菜单，并配置快捷键
* 修复 命令面板入口 未安装Git时，没有提示用户安装Git命令行工具的Bug
* 补充 EasyGit.searchGithubToClone 用于在【命令面板】快捷打开Clone视图在Github搜索

## 1.7.1
* 新增 git grep功能，支持在Git整个仓库搜索，比如搜索其它分支 （项目管理器，右键菜单【在git仓库中查找字符串】）
* 调整 分支视图 当前分支，增加merge图标 以方便分支合并操作
* 调整 blame 支持选择以何种方式呈现
* 调整 git储藏窗口UI
* 优化 当本地未安装Git时，提示弹窗，增加下载URL

## 1.7.0
* 优化 Git初始化仓库后 支持直接托管到Gitee、Github，需要Git服务器授权
* 新增 源代码管理器视图 点击项目名称 快速打开Git项目列表
* 调整 编辑器文件 右键菜单DIFF菜单
* 新增 Git文件对比视图 支持previous和next，以及指定commit对比
* 调整 Git文件对比视图，左上角增加刷新按钮
* 优化 Git文件对比视图，支持左右拖动
* 调整 Git日志视图，菜单背景等UI
* 优化 Git日志视图，查看文件详情 支持上下拖动
* 重构 SSH一键生成界面UI
* 优化 源代码管理器视图 自动fetch操作 错误消息改为状态栏提示
...
* 优化 源代码管理器视图性能
* 优化 源码视图和分支视图 输入框背景色
* 修复 雅蓝主题 源代码管理器视图 颜色没有跟项目管理器视图颜色保持一致的Bug
...
* 修复 git restore --staged操作，导致源代码管理器整个视图被刷新的Bug
* 调整 git reset操作 增加确认提示框

## 1.5.3.20211212
* 修复 克隆视图 某些情况下，无法显示Github搜索结果的Bug
* 优化 git commit操作因未设置user.name和user.email失败时，控制台给出解决方法
* 调整 git命令行2.29以上版本 windows credential.helper由manager调整为manager-core
...
* 修复 某些情况下，因数据量很大，长时间没有反应的Bug（目前先显示UI，再加载数据）
* 修复 源代码管理器、日志、分支视图等 切换主题 颜色没有随之变化的Bug
* 修复 源代码管理器视图 底部切换到上一个分支 某些情况下，没有提示错误的Bug
* 修复 当项目管理器内项目为git项目子目录时, easy-git相关视图，打开文件失败、获取文件修改错误的Bug （感谢用户`看见你的瞬间`的反馈）
